/* A filtering search form.
 *
 * The form takes input from a text field and filters elements in the wrapper
 * element set, showing only matching elements and hiding the rest in the set.
 */
(function (mu, $) {
    ...
    $.fn.muSearchForm = function (opts) {
        ...
        function filterElements(searchFor) {
            ...
            if (searchFor.length > 0) {
                searchScope.hide();
                searchScope.filter(":contains(" + searchFor + ")").show();
            }
            else {
                searchScope.show();
            }
